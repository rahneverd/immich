...
import { ImmichEnvironment, ImmichTelemetries, ImmichWorker, LogLevel } from 'src/enum';
...
const TELEMETRY_TYPES = new Set(Object.values(ImmichTelemetries));
...
const getEnv = (): EnvData => {
  ...
  const telemetryInclude =
    process.env.IMMICH_TELEMETRY_INCLUDE === 'all'
      ? Object.values(ImmichTelemetries).join(',')
      : process.env.IMMICH_METRICS_INCLUDE;
  const includedTelemetries = asSet(telemetryInclude, [] as ImmichTelemetries[]);
  const excludedTelemetries = asSet(process.env.IMMICH_TELEMETRY_EXCLUDE, [] as ImmichTelemetries[]);
  ...
  return {
    ...
    otel: {
      metrics: {
        hostMetrics: telemetries.has(ImmichTelemetries.HOST),
        apiMetrics: {
          enable: telemetries.has(ImmichTelemetries.API),
    ...
    telemetry: {
      ...
      enabled: telemetries.size > 0,
      hostMetrics: telemetries.has(ImmichTelemetries.HOST),
      apiMetrics: telemetries.has(ImmichTelemetries.API),
      repoMetrics: telemetries.has(ImmichTelemetries.REPO),
      jobMetrics: telemetries.has(ImmichTelemetries.JOB),
    },
